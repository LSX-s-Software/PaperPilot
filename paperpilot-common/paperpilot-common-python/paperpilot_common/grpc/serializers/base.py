...
class BaseModelSerializer:
    ...
    def _to_dict(self, grpc_fields, instance):
        return {name: self._get_field_value(grpc_field, name, instance) for name, grpc_field in grpc_fields}
    ...
    def _get_field_value(self, grpc_field, name, instance):
        ...
        if isinstance(field_meta, ForeignObjectRel):
            ...
            return self.serialize_model(self.get_grpc_message_class(grpc_field), field_value, self.serializers)
        else:
            return field_value
    ...
    def _serialize_model_relations(cls, grpc_field, related_items, serializers):
        ...
        return [cls.serialize_model(message_class, it, serializers) for it in related_items]
...
def message_to_python(message) -> dict:
    ...
    """
    return {field.name: _message_value(val) for field, val in message.ListFields()}
...
def _message_value(val):
    ...
    if class_name == "RepeatedCompositeContainer":
        return [message_to_python(it) for it in val]
